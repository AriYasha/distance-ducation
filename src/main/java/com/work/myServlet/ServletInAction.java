...
public class ServletInAction {
    ...
    public String chooseSelect(@RequestParam("name") String name, @ModelAttribute("user") UsersEntity usersEntity, Model model) {
    ...
    public String getRole(@ModelAttribute("user") UsersEntity usersEntity) {
        String page;
        if (usersEntity.getRole().equals("admin")) {
    ...
    public String getResultTestOne(@RequestParam("chooseOne") String chooseOne,
        ...
        switch (variable) {
            ...
            case (2):
                ...
                break;
            case (3):
                resultEntity.setNameTest("Физическая среда передачи данных");
                rating = 10 / 8 * (8 - count);
                resultEntity.setRatingTest(rating);
                break;
            case (4):
